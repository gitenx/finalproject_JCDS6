...
input_groups = dbc.FormGroup(
    [
        ...
        dbc.Input(placeholder='1.7', type='number', min=0, id='dbilirubin', value='0', step='0.01'),

        dbc.Label('Indirect Bilirubin'),
        dbc.Input(placeholder='10.7', type='number', min=0, id='ibilirubin', value='0', step='0.01'),

        dbc.Label('Alkaline Phosphotase'),
        dbc.Input(placeholder='177', type='number', min=0, id='alka', value='0', step='0.01'),

        dbc.Label('Alamine Aminotransferase'),
        dbc.Input(placeholder='109', type='number', min=0, id='alam', value='0', step='0.01'),

        dbc.Label('Aspartate Aminotransferase'),
        dbc.Input(placeholder='18', type='number', min=0, id='aspar', value='0', step='0.01'),

        dbc.Label('Total Proteins'),
        dbc.Input(placeholder='7.1', type='number', min=0, id='tprotein', value='0', step='0.01'),

        dbc.Label('Albumin'),
        dbc.Input(placeholder='2.6', type='number', min=0, id='albumin', value='0', step='0.01'),
            ...
            [
                dbc.Col(
                    [
                        ...
                        html.A('Kaggle: Indian Liver Patient Records',
                               href='https://www.kaggle.com/uciml/indian-liver-patient-records'),
                            ...
                            [
                                dbc.Card(
                                    [
                                        ...
                                        dbc.Collapse(
                                            dbc.CardBody([how_to, html.A('Sumber',
                                                                         href='https://hellosehat.com/hidup-sehat/tips-sehat/5-cara-menjaga-kesehatan-hati/')]),
                                            id='collapse-3',
                                        ),
                                    ]
                                )
...
def predict_liver(n_clicks, age, gender, ibilirubin, dbilirubin, alka, alam, aspar, tprotein, albumin):
    if n_clicks is None:
        raise dash.exceptions.PreventUpdate

    pred_model = pickle.load(open('model_ilp', 'rb'))

    if age == '0' or ibilirubin == '0' or dbilirubin == '0' or alka == '0' or alam == '0' or aspar == '0' or tprotein == '0' or albumin == '0' or age == '' or ibilirubin == '' or dbilirubin == '' or alka == '' or alam == '' or aspar == '' or tprotein == '' or albumin == '':
        children = dbc.Alert('You can\'t submit null or zero value!', color='danger')
    else:
        if float(tprotein) > float(albumin):
            nodisease = pred_model.predict_proba([[float(age), int(gender), float(ibilirubin) + float(dbilirubin),
                                                   float(dbilirubin), float(ibilirubin), float(alka), float(alam),
                                                   float(aspar), float(tprotein), float(albumin),
                                                   float(tprotein) / float(albumin)]])[0][0]
            disease = pred_model.predict_proba([[float(age), int(gender), float(ibilirubin) + float(dbilirubin),
                                                 float(dbilirubin), float(ibilirubin), float(alka), float(alam),
                                                 float(aspar), float(tprotein), float(albumin),
                                                 float(tprotein) / float(albumin)]])[0][1]
            pds = pred_model.predict([[float(age), int(gender), float(ibilirubin) + float(dbilirubin),
                                       float(dbilirubin), float(ibilirubin), float(alka), float(alam), float(aspar),
                                       float(tprotein), float(albumin), float(tprotein) / float(albumin)]])
            ...
            if pds == 1:
                ...
                children = dbc.Alert('The patient probably doesn\'t have liver disease with probability: {:0.2f}'.format(nodisease), color='success')
                # children = dbc.Alert('The patient doesn\'t have liver disease', color='success')
        else:
            children = dbc.Alert('Total protein must higher than albumin!', color='danger')
